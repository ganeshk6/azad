...
const Index = () => {
    ...
    const fetchWords = async (languageId) => {
        try {
            const response = await axios.get(route('correct-spelling-getWords'), {
                params: {
                    languageId,
                },
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(route('correct-spelling'), {
                letter,
                languageId,
            });
            ...
            if (response.status === 200) {
                ...
                window.location.href = route('correct-spelling-edit', { id: wordId });
                modalInstance.hide();
            }
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this word?')) {
            setIsLoading(true);
            try {
                const response = await axios.delete(route('correct-spelling-delete', { id }));
                if (response.status === 200) {
                            ...
                            {words.length > 0 && words.map((word) => (
                                <tr key={word.id}>
                                    ...
                                    <button
                                        ...
                                        >
                                            <Link
                                                href={route('correct-spelling-edit', { id: word.id })}
                                            >
                                                Edit
                                            </Link>
                                        </button>

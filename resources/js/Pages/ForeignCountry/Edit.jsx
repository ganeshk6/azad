...
const Edit = ({ phrasesData }) => {
    ...
    const [wordSection, setWordSections] = useState(phrasesData.SubForeignCountry);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            wordSection.forEach((section, index) => {
                formData.append(`SubForeignCountry[${index}][id]`, section.id);
                formData.append(`SubForeignCountry[${index}][title]`, section.title);
                if (section.image instanceof File) {
                    formData.append(`SubForeignCountry[${index}][image]`, section.image);
                }
            });
    
            const response = await axios.post(route('foreign-contries-edit', getPhrase.id), formData, {
                        ...
                        <div className="col-10 bg-white shadow-md rounded-lg p-6 relative">
                            ...
                            <div>
                                <div className='d-flex justify-between mb-3 align-items-center'>
                                    <label className="form-label h5 fw-bold">Type Foreign Country</label>
                                    <button
                                        type="button"
                                    ...
                                    {wordSection.map((section, index) => (
                                        <div className="accordion-item" key={section.id}>
                                            <h2 className="accordion-header d-flex" id={`heading-${section.id}`}>
                                                ...
                                                >
                                                    {section.title ? section.title : 'Foreign Country Section'} {index + 1}
                                                </button>
                                                <button
                                                    type="button"

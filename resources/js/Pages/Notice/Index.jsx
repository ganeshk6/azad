...
const Index = () => {
    ...
    const fetchWords = async (languageId) => {
        try {
            const response = await axios.get(route('notices-getWords'), {
                params: {
                    languageId,
                },
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(route('notices'), {
                letter,
                languageId,
            });
            ...
            if (response.status === 200) {
                ...
                window.location.href = route('notices-edit', { id: wordId });
                modalInstance.hide();
            }
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this word?')) {
            setIsLoading(true);
            try {
                const response = await axios.delete(route('notices-delete', { id }));
                if (response.status === 200) {
                            ...
                            {words.length > 0 && words.map((word) => (
                                <tr key={word.id}>
                                    ...
                                    <button
                                        ...
                                        >
                                            <Link
                                                href={route('notices-edit', { id: word.id })}
                                            >
                                                Edit
                                            </Link>
                                        </button>

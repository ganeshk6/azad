...
const Index = () => {
    ...
    const fetchWords = async (languageId) => {
        try {
            const response = await axios.get(route('outlines-getWords'), {
                params: {
                    languageId,
                },
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(route('outlines'), {
                sentence,
                languageId,
            });
            ...
            if (response.status === 200) {
                ...
                window.location.href = route('outlines-edit', { id: wordId });
                modalInstance.hide();
            }
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this word?')) {
            setIsLoading(true);
            try {
                const response = await axios.delete(route('outlines-delete', { id }));
                if (response.status === 200) {
                            ...
                            {words.length > 0 && words.map((word) => (
                                <tr key={word.id}>
                                    ...
                                    <button
                                        ...
                                        >
                                            <Link
                                                href={route('outlines-edit', { id: word.id })}
                                            >
                                                Edit
                                            </Link>
                                        </button>

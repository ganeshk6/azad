...
const Edit = ({ dictation }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            formData.append('description', getDictation.description);
            if (imageFile) {
                formData.append('image', imageFile);
            }

            await axios.post(route('rulesOutlines-edit', getDictation.id), formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

        } catch (error) {
            console.error('Error updating rulesOutlines:', error);
            alert('Failed to update rulesOutlines. Please try again.');
        } finally {
            setIsLoading(false);
        }
    };
                    ...
                    <div className="row">
                        <div className="col-10 bg-white shadow-md rounded-lg p-6 relative">
                            ...
                            <CKEditor
                                ...
                                data={getDictation.description}
                                config={{ licenseKey: 'GPL' }}
                                onChange={(event, editor) => {
                                    const data = editor.getData();
                                    setDictation({ ...getDictation, description: data });
                                }}
                            />

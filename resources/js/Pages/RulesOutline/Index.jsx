...
const Index = () => {
    ...
    const fetchWords = async (languageId) => {
        try {
            const response = await axios.get(route('rulesOutlines-getWords'), {
                params: {
                    languageId,
                },
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(route('rulesOutlines'), {
                sentence,
                languageId,
            });
            ...
            if (response.status === 200) {
                ...
                window.location.href = route('rulesOutlines-edit', { id: wordId });
                modalInstance.hide();
            }
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this word?')) {
            setIsLoading(true);
            try {
                const response = await axios.delete(route('rulesOutlines-delete', { id }));
                if (response.status === 200) {
                            ...
                            {words.length > 0 && words.map((word) => (
                                <tr key={word.id}>
                                    ...
                                    <button
                                        ...
                                        >
                                            <Link
                                                href={route('rulesOutlines-edit', { id: word.id })}
                                            >
                                                Edit
                                            </Link>
                                        </button>
            ...
            <div className="modal-content">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <input
                        ...
                        placeholder="Enter Sentence title"
                    />
                    </div>

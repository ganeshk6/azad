...
const Index = () => {
    ...
    const fetchWords = async (languageId) => {
        try {
            const response = await axios.get(route('temples-getWords'), {
                params: {
                    languageId,
                },
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(route('temples'), {
                letter,
                languageId,
            });
            ...
            if (response.status === 200) {
                ...
                window.location.href = route('temples-edit', { id: wordId });
                modalInstance.hide();
            }
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this word?')) {
            setIsLoading(true);
            try {
                const response = await axios.delete(route('temples-delete', { id }));
                if (response.status === 200) {
                            ...
                            {words.length > 0 && words.map((word) => (
                                <tr key={word.id}>
                                    ...
                                    <button
                                        ...
                                        >
                                            <Link
                                                href={route('temples-edit', { id: word.id })}
                                            >
                                                Edit
                                            </Link>
                                        </button>
